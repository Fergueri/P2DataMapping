async function housingGraph() { // Fetch and parse the CSV data
    const url = "_CSV/HOUSINGvsCPI.csv";
    ...
    const datasets = [
        {
            label: 'Paid Rental Fees for Housing Using C.P.I.',
            data: csvData.map((row) => row['Rent on Interest']),
            backgroundColor: 'rgba(248, 227, 162, 1)',
            borderColor: 'rgba(248, 227, 162, 1)',
            borderWidth: 5,
            fill: false
        }, {
            label: 'Paid Rental Fees for Housing (Actual)',
            data: csvData.map((row) => row['Paid rental fees for housing']),
    ...
    const ctx = document.getElementById('housingGraph').getContext('2d');
    ...
    const options = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Comparison of Actual vs Inflation-Adjusted Rental Fees for Housing Expenditures in Toronto',
                font: {
                    size: 30,
                    family: "BebasNeue"
                },
