async function telecomGraph() { // Fetch and parse the CSV data
    const url = "https://fergueri.github.io/P2DataMapping/CSV/TELECOMvsCPI.csv";
    ...
    const datasets = [
        {
            label: 'Telecommunication Services Using C.P.I.',
            data: csvData.map((row) => row['Telecom using CPI']),
            backgroundColor: 'rgba(248, 227, 162, 1)',
            borderColor: 'rgba(248, 227, 162, 1)',
            borderWidth: 5,
            fill: false
        }, {
            label: 'Telecommunication Services (Actual)',
    ...
    const ctx = document.getElementById('telecomGraph').getContext('2d');
    ...
    const options = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Comparison of Actual vs Inflation-Adjusted Telecommunication Service Expenditures in Toronto',
                font: {
                    size: 30,
                    family: "BebasNeue"
                },
