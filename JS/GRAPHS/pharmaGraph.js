async function pharmaGraph() { // Fetch and parse the CSV data
    const url = "https://fergueri.github.io/P2DataMapping/CSV/PHARMAvsCPI.csv";
    ...
    const datasets = [
        {
            label: 'Pharmaceuticals Using C.P.I.',
            data: csvData.map((row) => row['Pharma using CPI']),
            backgroundColor: 'rgba(248, 227, 162, 1)',
            borderColor: 'rgba(248, 227, 162, 1)',
            borderWidth: 5,
            fill: false
        }, {
            label: 'Pharmaceuticals (Actual)',
    ...
    const ctx = document.getElementById('pharmaGraph').getContext('2d');
    ...
    const options = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Comparison of Actual vs Inflation-Adjusted Pharmaceutical Expenditures in Toronto',
                font: {
                    size: 30,
                    family: "BebasNeue"
                },
